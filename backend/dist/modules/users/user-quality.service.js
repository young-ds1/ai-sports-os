...
var UserQualityService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserQualityService = void 0;
const common_1 = require("@nestjs/common");
let UserQualityService = UserQualityService_1 = class UserQualityService {
    logger = new common_1.Logger(UserQualityService_1.name);
    users = new Map();
    shareRecords = [];
    ...
    trackUserAction(params) {
        ...
        switch (params.action) {
            ...
            case 'page_view':
                ...
                break;
            case 'paywall_view':
                user.sawPaywall = true;
                break;
            case 'upgrade_click':
                user.clickedUpgrade = true;
                break;
            case 'payment':
                user.paid = true;
                break;
    ...
    getChannelQualityReport() {
        ...
        for (const user of this.users.values()) {
            if (!byChannel.has(user.channel))
                byChannel.set(user.channel, []);
        ...
        for (const [channel, users] of byChannel) {
            ...
            let verdict;
            if (avgScore >= 50 && highPct >= 30)
                verdict = 'premium_channel';
            else if (users.length >= 20 && avgScore < 30)
                verdict = 'volume_channel';
            else if (avgScore >= 40)
                verdict = 'mixed';
            else
                verdict = 'poor';
    ...
    getContentQualityReport() {
        ...
        for (const user of this.users.values()) {
            if (!byExperiment.has(user.contentExperiment))
                byExperiment.set(user.contentExperiment, []);
        ...
        for (const [experiment, users] of byExperiment) {
            ...
            let verdict;
            if (aiPerUser >= 2.0 && retention >= 30)
                verdict = 'high_engagement';
            else if (registrations >= 10 && aiPerUser < 1.0)
                verdict = 'high_ctr_low_engagement';
            else
    ...
    getShareLoopAnalysis() {
        ...
        const topSharing = Array.from(byContent.entries())
            ...
            experiment, shares,
            invitedQuality: Math.round(this.shareRecords
...
exports.UserQualityService = UserQualityService;
exports.UserQualityService = UserQualityService = UserQualityService_1 = __decorate([
    (0, common_1.Injectable)()
], UserQualityService);
//# sourceMappingURL=user-quality.service.js.map