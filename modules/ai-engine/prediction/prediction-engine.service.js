...
let PredictionEngineService = PredictionEngineService_1 = class PredictionEngineService {
    ...
    predictMatch(homeTeam, awayTeam) {
        ...
        const confidence = Math.round(Math.min(95, Math.max(30, 50 + (eloGap > 100 ? 25 : eloGap > 50 ? 15 : 5) + (home.formScore > 70 ? 10 : 0) + (away.formScore > 70 ? 10 : 0))));
    ...
    generateScoreProbs(homeStrength, awayStrength) {
        // Simplified Poisson-based score probabilities
        const scores = [];
        for (let h = 0; h <= 3; h++) {
            for (let a = 0; a <= 3; a++) {
                if (h === 0 && a === 0)
                    continue; // Skip 0-0 for top 5
    ...
    generateExplanation(home, away, hProb, dProb, aProb, eloDiff) {
        const parts = [];
        if (eloDiff > 80) {
        ...
        if (home.formScore > 80)
            parts.push(`${home.team} 近期状态出色（${home.form}）。`);
        if (away.formScore > 80)
            parts.push(`${away.team} 近期状态火热（${away.form}）。`);
