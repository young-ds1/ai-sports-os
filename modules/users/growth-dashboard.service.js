...
var GrowthDashboardService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.GrowthDashboardService = void 0;
...
let GrowthDashboardService = GrowthDashboardService_1 = class GrowthDashboardService {
    userUsageService;
    marketValidation;
    logger = new common_1.Logger(GrowthDashboardService_1.name);
    visitorLog = [];
    constructor(userUsageService, marketValidation) {
    ...
    // ── Growth Dashboard ──
    async getAcquisitionSnapshot(days = 7) {
        ...
        for (const v of visitors) {
            if (!bySource[v.source])
                bySource[v.source] = { visitors: 0, signups: 0, aiUsers: 0, payingUsers: 0, conversionRate: 0, costPerAcquisition: '$0' };
            bySource[v.source].visitors++;
            if (v.signedUp)
                bySource[v.source].signups++;
            if (v.didAiAction)
                bySource[v.source].aiUsers++;
            if (v.didPay)
                bySource[v.source].payingUsers++;
            if (!byCampaign[v.campaign])
                byCampaign[v.campaign] = { visitors: 0, signups: 0 };
            byCampaign[v.campaign].visitors++;
            if (v.signedUp)
                byCampaign[v.campaign].signups++;
            if (!byContentType[v.contentType])
                byContentType[v.contentType] = { pieces: 0, visitors: 0, avgEngagement: 0 };
    ...
    async dailyContentCron() {
        const plan = this.getDailyContentPlan();
        this.logger.log(`📅 [Daily Content] ${plan.date}: ${plan.matches.length} matches → ` +
        ...
        for (const m of plan.matches.filter(m => m.interestScore >= 80)) {
            this.logger.log(`  🔥 ${m.homeTeam} vs ${m.awayTeam} (score=${m.interestScore}): ` +
                `${m.contentGenerated.join(', ')}`);
        }
    }
    seedBenchmarkData(bySource, byContentType) {
...
exports.GrowthDashboardService = GrowthDashboardService;
exports.GrowthDashboardService = GrowthDashboardService = GrowthDashboardService_1 = __decorate([
...
//# sourceMappingURL=growth-dashboard.service.js.map