...
// ── STEP 1-2: Segment Role Assignment ──
...
const SEGMENT_ROLES: SegmentRole[] = [
  {
...
// ── STEP 4: Future Measurable Events ──
...
const FLYWHEEL_EVENTS: FlywheelEvent[] = [
  {
...
// ── STEP 5: Flywheel Dashboard Schema ──
export interface FlywheelDashboardSchema {
  ...
  conversion_by_segment: {
    schema: Record<string, { from: string; to: string; rate: string }>;
    description: 'Conversion rates between segments (e.g., Fan → Fantasy)';
  };
  revenue_by_segment: {
    schema: Record<string, { total: string; per_user: string; mrr: string }>;
    description: 'Revenue contribution by segment';
  };
  retention_by_segment: {
    schema: Record<string, { d7: string; d30: string; avg_sessions: string }>;
...
const DASHBOARD_SCHEMA: FlywheelDashboardSchema = {
  _note: 'ALL VALUES ARE PLACEHOLDERS. COMPUTED ONLY WHEN VALIDATION GATES ARE MET.',
  user_inflow_by_segment: {
...
// ── Master Flywheel Hypothesis Document ──
...
@Injectable()
export class FlywheelDesignService {
