...
export interface MoatAssessment {
  moats: MoatHealth[];
  overallMoatScore: number;    // 0-100, weighted average
  defensibilityRating: string; // 'weak' | 'developing' | 'strong' | 'impregnable'
  dataAccumulationRate: number; // new data points per day
...
export interface MoatHealth {
  ...
  keyMetric: string;
  metricValue: string;
  evidence: string[];          // Why this moat is real
  vcNarrative: string;         // How to pitch this moat to investors
}
...
export class MoatMetricsService {
  ...
  assess(metrics: {
    ...
    const overall = Math.round(
      moats.reduce((sum, m) => sum + m.score, 0) / moats.length,
    );
   ...
   */
  private assessEventGraph(m: any): MoatHealth {
   ...
   */
  private assessBehavioralFlywheel(m: any): MoatHealth {
   ...
   */
  private assessAdaptivePrediction(m: any): MoatHealth {
  ...
  private estimateSwitchingCost(m: any): string {
    const dataPoints = m.totalMatches + m.totalEvents + m.totalAiAnalyses + m.totalChatMessages;
    if (dataPoints > 10000) return '极高 — 10K+ 结构化数据点，完整用户行为图谱，迁移成本 > $500K';
    if (dataPoints > 1000) return '高 — 1K+ 数据点，用户习惯已形成，迁移成本 > $50K';
    if (dataPoints > 100) return '中等 — 数据积累中，早期用户锁定进行中';
