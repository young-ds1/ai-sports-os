 ...
 */
export class ApiFootballMockAdapter implements IProviderAdapter {
  ...
  async getSeasons(competitionProviderId: number): Promise<RawSeason[]> {
    return [
      { name: '2026', start_date: '2026-06-12', end_date: '2026-07-19', is_current: true },
    ];
  }

  async getTeams(_competitionProviderId: number, _season: string): Promise<RawTeam[]> {
    return [
  ...
  async getPlayers(_teamProviderId: number): Promise<RawPlayer[]> {
    return [
  ...
  async getFixtures(date: string): Promise<RawMatch[]> {
    return [
      {
  ...
  async getMatchDetail(matchProviderId: number): Promise<{ match: RawMatch; events: RawMatchEvent[] }> {
    return {
      match: {
  ...
  async getLiveMatches(): Promise<RawMatch[]> {
  ...
  async getStandings(_competitionProviderId: number, _season: string): Promise<RawStanding[]> {
    return [
